view: growth_1995_2005 {
  derived_table: {
    sql: SELECT
        IFNULL(a.upper_latin, b.upper_latin) as upper_latin,
        IFNULL(count_2005, 0) as count_2005,
        IFNULL(count_1995, 0) as count_1995,
        (IFNULL(count_2005, 0)-IFNULL(count_1995, 0)) AS count_growth,
        (IFNULL(alive_2005, 0)-IFNULL(alive_1995, 0)) as alive_growth,
        (IFNULL(dead_2005, 0)-IFNULL(dead_1995, 0)) as dead_growth
      FROM (
        SELECT
          UPPER(spc_latin) AS upper_latin,
          spc_common,
          COUNT(*) AS count_2005,
          COUNTIF(status="Alive") AS alive_2005,
